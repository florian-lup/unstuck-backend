...
class DatabaseService:
        ...
        try:
            ...
            query = select(Conversation).where(
                and_(
                    ...
                    Conversation.user_id == user_id  # Security: user can only access their own data
                )
            ).options(
    ...
    async def add_message(
        ...
        try:
            ...
            if not conversation:
                logger.warning(f"User {user_id} attempted to add message to conversation {conversation_id} they don't own")
                return None
    ...
    async def delete_conversation(
        ...
        try:
            ...
            if conversation:
                ...
                logger.info(f"User {user_id} permanently deleted conversation {conversation_id}")
                return True
