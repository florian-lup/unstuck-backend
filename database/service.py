...
class DatabaseService:
    ...
    async def increment_voice_requests(self, user_id: UUID) -> User:
        """
        Increment user VOICE CHAT request counters using atomic SQL operations.

        For free tier: increments total_voice_requests (lifetime)
        For community tier: increments monthly_voice_requests (resets monthly)
        ...
        try:
            ...
            # For community tier, only track monthly voice requests
            if user.subscription_tier == "community":
                # Initialize or check if we need to reset monthly counter
                if user.voice_request_count_reset_date is None:
                    # First time tracking monthly voice requests - reset counter
                    stmt = (
                        update(User)
                        .where(User.id == user_id)
                        .values(
                            monthly_voice_requests=1,
                            voice_request_count_reset_date=current_time,
                            updated_at=current_time,
                        )
                    )
                    await self.db.execute(stmt)
                    ...
                    # Check if a month has passed since last reset
                    days_since_reset = (
                        current_time - user.voice_request_count_reset_date
                    ).days
                    if days_since_reset >= 30:
                        # Reset monthly counter
                        stmt = (
                        ...
                        stmt = (
                            ...
                            .where(User.id == user_id)
                            .values(
                                monthly_voice_requests=User.monthly_voice_requests + 1,
                                updated_at=current_time,
                            )
                        )
                        await self.db.execute(stmt)
            else:
                # For free tier and other tiers, only increment total_voice_requests
                stmt = (
                    update(User)
                    .where(User.id == user_id)
                    .values(
                        total_voice_requests=User.total_voice_requests + 1,
                        updated_at=current_time,
                    )
                )
                await self.db.execute(stmt)
            ...
            logger.error(f"Error incrementing voice requests: {e}")
            await self.db.rollback()
            raise
