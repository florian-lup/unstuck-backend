...
def _check_voice_request_limit(user: User) -> None:
    """
    Check if user has exceeded their VOICE CHAT request limit.
    ...
    if tier == "free":
        # Free tier: Check lifetime voice chat limit
        max_requests = cast(
            int, SUBSCRIPTION_LIMITS[SubscriptionTier.FREE]["max_total_voice_requests"]
        )
        if user.total_voice_requests >= max_requests:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail={
                    "error": "voice_request_limit_exceeded",
                    "message": (
                        f"You've used all **{max_requests} voice chat requests** on the **{tier} tier**.\n\n"
                        "✨ **Upgrade to Community tier to continue using voice chat:**\n"
                        "• 150 voice chat requests per month\n"
                        "• Monthly limit resets automatically\n"
                        "• Support development of Unstuck\n\n"
                        '💡 *Click the settings icon and select "Upgrade Subscription" to continue!*'
                    ),
                    "current_requests": user.total_voice_requests,
                    "max_requests": max_requests,
                    "tier": tier,
                    "limit_type": "lifetime",
        ...
        # Community tier: Check monthly voice chat limit
        max_monthly = cast(
            int, SUBSCRIPTION_LIMITS[SubscriptionTier.COMMUNITY]["max_monthly_voice_requests"]
        )

        # Check if we need to reset monthly counter
        current_time = datetime.now(UTC)
        if user.voice_request_count_reset_date is None:
            # First request, will be reset in increment_voice_requests
            pass
        else:
            days_since_reset = (current_time - user.voice_request_count_reset_date).days
            if days_since_reset >= 30:
                # Counter will be reset in increment_voice_requests
                pass
            elif user.monthly_voice_requests >= max_monthly:
                # Exceeded monthly limit
                days_until_reset = 30 - days_since_reset
                reset_date_str = (
                    user.voice_request_count_reset_date.strftime("%B %d, %Y")
                    if user.voice_request_count_reset_date
                    else "soon"
                )
                day_text = "day" if days_until_reset == 1 else "days"
                ...
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail={
                        "error": "monthly_voice_request_limit_exceeded",
                        "message": (
                            f"You've used all **{max_monthly} voice chat requests** this month on the **{tier} tier**.\n\n"
                            f"⏰ Your limit will reset in **{days_until_reset} {day_text}** (on {reset_date_str}).\n\n"
                            "✨ **Or upgrade to Pro tier for unlimited voice chat:**\n"
                            "• Unlimited requests every month\n"
                            "• Exclusive features (coming soon)"
                        ),
                        "current_requests": user.monthly_voice_requests,
                        "max_requests": max_monthly,
                        "tier": tier,
                        "limit_type": "monthly",
                        "request_type": "voice_chat",
                        "days_until_reset": days_until_reset,
                        "reset_date": user.voice_request_count_reset_date.isoformat()
                        if user.voice_request_count_reset_date
                        else None,
                    },
                )
...
def get_voice_request_limit_info(user: User) -> dict[str, int | str | None]:
    """
    Get VOICE CHAT request limit information for a user.
    ...
    if tier == "free":
        ...
        remaining = max(0, max_requests - user.total_voice_requests)
        return {
            "remaining_requests": remaining,
            "max_requests": max_requests,
            "limit_type": "lifetime",
    ...
    if tier == "community":
        ...
        if user.voice_request_count_reset_date is None:
            ...
            if days_since_reset >= 30:
                ...
                remaining = max(0, max_monthly - user.monthly_voice_requests)
                # Calculate next reset date (30 days from last reset)
                next_reset = user.voice_request_count_reset_date.replace(
                    day=user.voice_request_count_reset_date.day
                )
                # Add 30 days
                import calendar
...
async def check_voice_request_limits_only(
    current_user: AuthenticatedUser = Depends(get_current_user),  # noqa: B008
    db_session: AsyncSession = Depends(get_db_session),  # noqa: B008
) -> User:
    """
    Check VOICE CHAT request limits only (no feature restrictions) and increment counter.

    Use this for voice chat features that are available to all tiers but still count towards limits.
    ...
    Returns:
        User: The user database record after incrementing voice request counter

    Raises:
        HTTPException: If user has exceeded voice chat request limits
    """
    # Get user from database
    db_service = DatabaseService(db_session)
    user = await db_service.get_or_create_user(
    ...
    # Check voice request limits
    _check_voice_request_limit(user)

    # Increment voice request counter
    return await db_service.increment_voice_requests(user.id)
